...
const cases = [
  {
    ...
    steps: [
      ...
      '3. Open Full Menu path: Monitoring > Real User Browser > Performance Overview (or navigate via route)',
      '4. Wait for the page to finish loading',
      '5. Observe page title/breadcrumb and browser URL',
    ].join('\n'),
    expected: [
      'Page title includes Performance Overview and/or Core Web Vitals (VitalScope)',
  ...
  },
  {
    ...
    steps: [
      '1. Open RUM Performance Overview for GDC Test Site 2',
  ...
  },
  {
    ...
    steps: [
      '1. On Performance Overview, locate the Performance by Page table',
...
async function main() {
  ...
  summary.addRow(['RUM Performance Overview — Regression Summary']);
  ...
  notes.addRow(['Blue Triangle Portal — RUM Performance Overview (Browser) Regression Test Cases']);
  notes.addRow([`Profile / Site: ${DC} — ${SITE}`]);
  notes.addRow(['Type: Regression. Customize Table Save & Close allowed. Do NOT Save Filter.']);
  notes.addRow([
  ...
  const outPath = path.join(outDir, 'RUM_Performance_Overview_Regression.xlsx');
