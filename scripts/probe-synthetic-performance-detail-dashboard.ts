/**
 * Live probe: Synthetic Performance Detail preconfigured dashboard (site/dashboard)
 * NOT Monitoring > Synthetic > Real Browser > Performance Detail
 * NOT Synthetic Site Health
 * Run: npx tsx scripts/probe-synthetic-performance-detail-dashboard.ts
 */
...
const out = path.join(outDir, 'synthetic-performance-detail-dashboard-live-probe.json');
...
const SNAPSHOT_JS = `(() => {
  ...
  const interesting = allIds
    .filter((id) =>
      /dashboard|widget|chart|graph|filter|time|lookback|refresh|period|timezone|highchart|metric|switch|auto|marker|table|card|export|legend|onload|daily|average|page|timing|detail|bar|cwv|lcp|cls|inp|synth|avail|http|error|slick|gear|setting|probe|monitor/i.test(
        id
      )
    )
    .slice(0, 500);
...
const SPD_EXACT = /^\s*Synthetic Performance Detail\s*$/i;
const SPD_ALT = /^\s*Synthetic\s+Performance\s+Detail\s+Dashboard\s*$/i;

function isSyntheticPerformanceDetailLabel(text: string): boolean {
  const t = text.replace(/\s+/g, ' ').trim();
  // Must NOT match RUM, Site Health, Native App, Overview, etc.
  if (/\bRUM\b|site\s*health|native\s*app|overview|aggregate/i.test(t)) return false;
  return SPD_EXACT.test(t) || SPD_ALT.test(t);
}

async function selectSyntheticPerformanceDetail(page: Page): Promise<boolean> {
  ...
  if ((await native.count()) > 0) {
    ...
    const match = labels.find((t) => isSyntheticPerformanceDetailLabel(t.replace(/\s+/g, ' ').trim()));
    if (match) {
      await native.selectOption({ label: match }).catch(async () => {
        const val = await native
          .locator('option')
          .filter({ hasText: /^\s*Synthetic Performance Detail\s*$/i })
          .first()
          .getAttribute('value');
...
async function main() {
  ...
  try {
    ...
    const selected = await selectSyntheticPerformanceDetail(page);
    await dismiss(page);
    ...
    let dashNow = (
      ...
      .replace(/\s+/g, ' ')
      .trim();
    if (!isSyntheticPerformanceDetailLabel(dashNow)) {
      await selectSyntheticPerformanceDetail(page);
      await page.waitForTimeout(8000);
      await ensureGdc(page);
    }

    // Soft refresh once if site/subtitle may lag
    await page.locator('#refresh-dashboard').click({ force: true }).catch(() => undefined);
    ...
    save({
      afterSyntheticPerformanceDetailSelect: {
        selected,
        dashAfter: dashNow,
        siteAfter,
        snap: afterSelect,
    ...
    const shot = path.join(outDir, 'synthetic-performance-detail-dashboard-live.png');
    await page.screenshot({ path: shot, fullPage: true }).catch(() => undefined);

    const cardClick = page
      .locator('.grid-stack-item')
      .filter({ hasText: /Details|Onload|LCP|CLS|INP|Page Views|Availability|DNS/i })
    ...
    console.log('[probe-synth-pd-dashboard] wrote', out);
    console.log('[probe-synth-pd-dashboard] selected=', selected, 'dashboard=', dashNow, 'site=', siteAfter);
    console.log(
      '[probe-synth-pd-dashboard] widgets=',
