/**
 * Live probe: VitalPulse preconfigured dashboard
 * Run: npx tsx scripts/probe-vital-pulse.ts
 */
...
const out = path.join(outDir, 'vital-pulse-live-probe.json');
...
const SNAPSHOT_JS = `(() => {
  ...
  const interesting = allIds
    .filter((id) =>
      /dashboard|widget|chart|graph|map|geo|device|revenue|performance|vital|pulse|filter|time|lookback|refresh|period|timezone|highchart|metric|switch|auto|marker|table|card|export|legend|onload|lcp|cls|inp|session|page/i.test(
        id
      )
    )
    .slice(0, 320);
  ...
  const bodySample = txt(document.body).slice(0, 3500);
  return {
...
async function selectDashboardByText(page: Page, re: RegExp): Promise<boolean> {
  ...
  if (await container.isVisible().catch(() => false)) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (/VitalPulse|Vital Pulse/i.test(String(re)) && !/^VitalPulse$|^Vital Pulse$/i.test(text) && !/^VitalPulse$/i.test(text)) {
        if (!/^VitalPulse$/i.test(text)) continue;
      }
...
async function main() {
  ...
  try {
    ...
    const selected = await selectDashboardByText(page, /VitalPulse|Vital Pulse/i);
    await dismiss(page);
    await page.waitForTimeout(5000);
    const afterSelect = await page.evaluate(SNAPSHOT_JS);
    save({ afterVitalPulseSelect: { selected, snap: afterSelect } });

    await openFilters(page);
    const filters = await page.evaluate(SNAPSHOT_JS);
    save({ filtersOpen: filters });

    // Capture time lookback options without closing permanently
    await page.keyboard.press('Escape').catch(() => undefined);
    ...
    console.log('[probe-vital-pulse] wrote', out);
    console.log('[probe-vital-pulse] selected=', selected, 'dashboard=', afterSelect.select2?.find?.((s: { id: string }) => /switch-dashboard/i.test(s.id)));
  } finally {
    await browser.close();
  }
}
