/**
 * Live probe: Business Regression - US2 (custom/shared dashboard)
 * Nav: Dashboards → #switch-dashboard → exact "Business Regression - US2"
 * Run: npx tsx scripts/probe-business-regression-us2-dashboard.ts
 */
...
const DASHBOARD_EXACT = 'Business Regression - US2';
const DASHBOARD_RE = /^\s*Business Regression\s*-\s*US2\s*$/i;

const outDir = path.join(process.cwd(), 'docs', 'prompts');
const out = path.join(outDir, 'business-regression-us2-dashboard-live-probe.json');
const shot = path.join(outDir, 'business-regression-us2-dashboard-live.png');
...
async function selectDashboardExact(page: Page, exact: string, re: RegExp): Promise<{
  ...
  if (await container.isVisible().catch(() => false)) {
    ...
    if (await search.isVisible().catch(() => false)) {
      await search.fill('Business Regression').catch(() => undefined);
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Reject near-misses
      if (/Synthetic Regression|Alerts Regression|Marketing Regression|RUM Regression|Tag Governance/i.test(text) && !/^Business Regression/i.test(text))
        continue;
...
async function main() {
  ...
  try {
    ...
    console.log('[probe-business-regression-us2] wrote', out);
