/**
 * Live probe: Synthetic Regression - US2 (custom/shared dashboard)
 * Nav: Dashboards → #switch-dashboard → exact "Synthetic Regression - US2"
 * Run: npx tsx scripts/probe-synthetic-regression-us2-dashboard.ts
 */
...
const DASHBOARD_EXACT = 'Synthetic Regression - US2';
const DASHBOARD_RE = /^\s*Synthetic Regression\s*-\s*US2\s*$/i;

const outDir = path.join(process.cwd(), 'docs', 'prompts');
const out = path.join(outDir, 'synthetic-regression-us2-dashboard-live-probe.json');
const shot = path.join(outDir, 'synthetic-regression-us2-dashboard-live.png');
...
const SNAPSHOT_JS = `(() => {
  ...
  const allDashOptions = Array.from(document.querySelectorAll('#switch-dashboard option'))
    ...
    .filter((o) => o.text)
    .slice(0, 200);
  const syntheticHits = allDashOptions.filter((o) => /synthetic|regression|US2/i.test(o.text));
  const filterChrome = {
...
async function selectDashboardExact(page: Page, exact: string, re: RegExp): Promise<{
  ...
  if (await container.isVisible().catch(() => false)) {
    ...
    if (await search.isVisible().catch(() => false)) {
      await search.fill('Synthetic Regression').catch(() => undefined);
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Reject near-misses
      if (/Synthetic Site Health|Synthetic Performance Detail|Synthetic Performance Overview/i.test(text) && !/Regression/i.test(text))
        continue;
...
async function main() {
  ...
  try {
    ...
    console.log('[probe-synthetic-regression-us2] wrote', out);
