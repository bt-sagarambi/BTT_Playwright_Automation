/**
 * Live probe: Marketing Regression - US2 (custom/shared dashboard)
 * Nav: Dashboards → #switch-dashboard → exact "Marketing Regression - US2"
 * Run: npx tsx scripts/probe-marketing-regression-us2-dashboard.ts
 */
...
const DASHBOARD_EXACT = 'Marketing Regression - US2';
const DASHBOARD_RE = /^\s*Marketing Regression\s*-\s*US2\s*$/i;

const outDir = path.join(process.cwd(), 'docs', 'prompts');
const out = path.join(outDir, 'marketing-regression-us2-dashboard-live-probe.json');
const shot = path.join(outDir, 'marketing-regression-us2-dashboard-live.png');
...
const SNAPSHOT_JS = `(() => {
  ...
  const allDashOptions = Array.from(document.querySelectorAll('#switch-dashboard option'))
    ...
    .filter((o) => o.text)
    .slice(0, 200);
  const syntheticHits = allDashOptions.filter((o) => /synthetic|regression|US2|alert|announce|business|conversion|revenue|marketing|tag|rum/i.test(o.text));
  const filterChrome = {
...
async function selectDashboardExact(page: Page, exact: string, re: RegExp): Promise<{
  ...
  if (await container.isVisible().catch(() => false)) {
    ...
    if (await search.isVisible().catch(() => false)) {
      await search.fill('Marketing Regression').catch(() => undefined);
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Reject near-misses
      if (/Synthetic Regression|Alerts Regression|Business Regression|RUM Regression|Tag Governance/i.test(text) && !/^Marketing Regression/i.test(text))
        continue;
...
async function main() {
  ...
  try {
    ...
    console.log('[probe-marketing-regression-us2] wrote', out);
