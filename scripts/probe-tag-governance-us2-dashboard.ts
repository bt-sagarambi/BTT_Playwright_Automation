/**
 * Live probe: Tag Governance - US2 (custom/shared dashboard)
 * Nav: Dashboards → #switch-dashboard → exact "Tag Governance - US2"
 * Run: npx tsx scripts/probe-tag-governance-us2-dashboard.ts
 */
...
const DASHBOARD_EXACT = 'Tag Governance - US2';
const DASHBOARD_RE = /^\s*Tag Governance\s*-\s*US2\s*$/i;

const outDir = path.join(process.cwd(), 'docs', 'prompts');
const out = path.join(outDir, 'tag-governance-us2-dashboard-live-probe.json');
const shot = path.join(outDir, 'tag-governance-us2-dashboard-live.png');
...
async function selectDashboardExact(page: Page, exact: string, re: RegExp): Promise<{
  ...
  if (await container.isVisible().catch(() => false)) {
    ...
    if (await search.isVisible().catch(() => false)) {
      await search.fill('Tag Governance').catch(() => undefined);
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Reject near-misses
      if (/Synthetic Regression|Alerts Regression|Business Regression|Marketing Regression|RUM Regression/i.test(text) && !/^Tag Governance/i.test(text))
        continue;
...
async function main() {
  ...
  try {
    ...
    console.log('[probe-tag-governance-us2] wrote', out);
