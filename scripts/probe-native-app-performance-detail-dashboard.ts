/**
 * Live probe: Native App Performance Detail preconfigured dashboard (site/dashboard)
 * NOT Monitoring > Real User Browser > Performance Detail
 * Run: npx tsx scripts/probe-native-app-performance-detail-dashboard.ts
 */
...
const out = path.join(outDir, 'native-app-performance-detail-dashboard-live-probe.json');
...
const NAPD_EXACT = /^\s*Native App Performance Detail\s*$/i;
const NAPD_ALT = /^\s*Native App Performance Detail(s)?( Dashboard)?\s*$/i;

function isNativeAppPerformanceDetailLabel(text: string): boolean {
  const t = text.replace(/\s+/g, ' ').trim();
  // Must NOT match RUM/Synthetic PDs or Native App Detailed Metrics-* siblings.
  if (/synthetic|\brum\b|detailed metrics|overview|site health/i.test(t)) return false;
  return NAPD_EXACT.test(t) || NAPD_ALT.test(t);
}

async function selectNativeAppPerformanceDetail(page: Page): Promise<boolean> {
  ...
  if ((await native.count()) > 0) {
    ...
    const match = labels.find((t) => isNativeAppPerformanceDetailLabel(t.replace(/\s+/g, ' ').trim()));
    if (match) {
      await native.selectOption({ label: match }).catch(async () => {
        const val = await native
          .locator('option')
          .filter({ hasText: /^\s*Native App Performance Detail\s*$/i })
          .first()
          .getAttribute('value');
...
async function main() {
  ...
  try {
    ...
    const selected = await selectNativeAppPerformanceDetail(page);
    await dismiss(page);
    ...
    let dashNow = (
      ...
      .replace(/\s+/g, ' ')
      .trim();
    if (!isNativeAppPerformanceDetailLabel(dashNow)) {
      await selectNativeAppPerformanceDetail(page);
      await page.waitForTimeout(8000);
      await ensureGdc(page);
    }
    ...
    save({
      afterNativeAppPerformanceDetailSelect: {
        selected,
        dashAfter: dashNow,
        siteAfter,
        snap: afterSelect,
    ...
    const shot = path.join(outDir, 'native-app-performance-detail-dashboard-live.png');
    ...
    const cardClick = page
      ...
      .filter({ hasText: /Details|Onload|CPU|Memory|Crash|ANR|Page Views|HTTP|Network/i })
    ...
    const gear = page
      .locator('.grid-stack-item')
      .filter({ hasText: /Page Timings Over Time|Performance Graph|Native App|Crash|ANR|CPU|Memory|HTTP/i })
    ...
    console.log('[probe-native-app-pd-dashboard] wrote', out);
    console.log('[probe-native-app-pd-dashboard] selected=', selected, 'dashboard=', dashNow, 'site=', siteAfter);
    console.log(
      '[probe-native-app-pd-dashboard] widgets=',
