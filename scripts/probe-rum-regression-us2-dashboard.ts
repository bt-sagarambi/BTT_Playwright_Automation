/**
 * Live probe: RUM Regression - US2 (custom/shared dashboard)
 * Nav: Dashboards → #switch-dashboard → exact "RUM Regression - US2"
 * Run: npx tsx scripts/probe-rum-regression-us2-dashboard.ts
 */
...
const DASHBOARD_EXACT = 'RUM Regression - US2';
const DASHBOARD_RE = /^\s*RUM Regression\s*-\s*US2\s*$/i;

const outDir = path.join(process.cwd(), 'docs', 'prompts');
const out = path.join(outDir, 'rum-regression-us2-dashboard-live-probe.json');
const shot = path.join(outDir, 'rum-regression-us2-dashboard-live.png');
...
async function selectDashboardExact(page: Page, exact: string, re: RegExp): Promise<{
  ...
  if (await container.isVisible().catch(() => false)) {
    ...
    if (await search.isVisible().catch(() => false)) {
      await search.fill('RUM Regression').catch(() => undefined);
    ...
    for (let i = 0; i < count; i++) {
      ...
      // Reject near-misses
      if (
        /Synthetic Regression|Alerts Regression|Business Regression|Marketing Regression|Tag Governance/i.test(
          text
        ) &&
        !/^RUM Regression/i.test(text)
      )
        continue;
...
async function main() {
  ...
  try {
    ...
    console.log('[probe-rum-regression-us2] wrote', out);
