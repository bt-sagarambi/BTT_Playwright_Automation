...
import { NativeAppPerformanceDetailDashboardLocators } from '../locators/NativeAppPerformanceDetailDashboardLocators';
...
/** Exact preconfigured option — never RUM/Synthetic/Detailed Metrics. */
export const NAPD_DASH_EXACT = /^\s*Native App Performance Detail\s*$/i;
export const NAPD_DASH_ALT =
  /^\s*Native App Performance Details?(\s+Dashboard)?\s*$/i;

export function isNativeAppPerformanceDetailDashboardLabel(text: string): boolean {
  const t = (text || '').replace(/\s+/g, ' ').trim();
  if (/detailed\s*metrics/i.test(t)) return false;
  if (/^\s*RUM\s+Performance\s+Detail/i.test(t)) return false;
  if (/^\s*Synthetic\s+Performance\s+Detail/i.test(t)) return false;
  if (/overview/i.test(t) && !/performance\s+detail/i.test(t)) return false;
  return NAPD_DASH_EXACT.test(t) || NAPD_DASH_ALT.test(t);
}

export type NativeAppPerformanceDetailDashboardContext = {
...
/**
 * Preconfigured Native App Performance Detail dashboard (site/dashboard).
 * Read-only: no Save Filter / permanent widget save / dashboard delete.
 * NOT Monitoring → Native App → Performance Detail; NOT Detailed Metrics -*.
 */
export class NativeAppPerformanceDetailDashboardPage {
  readonly locators: NativeAppPerformanceDetailDashboardLocators;

  constructor(private readonly page: Page) {
    this.locators = new NativeAppPerformanceDetailDashboardLocators(page);
  }
  ...
  async openViaNavigation(): Promise<void> {
    ...
    await this.ensureNativeAppPerformanceDetailSelected();
  ...
  async recoverPage(): Promise<void> {
    ...
    await this.ensureNativeAppPerformanceDetailSelected({ soft: true });
    await this.ensureProfileSiteSelected();
    if (!isNativeAppPerformanceDetailDashboardLabel(await this.getDashboardLabel())) {
      await this.openViaNavigation();
    } else {
  ...
  async ensureNativeAppPerformanceDetailSelected(opts?: { soft?: boolean }): Promise<void> {
    const soft = opts?.soft === true;
    let label = await this.getDashboardLabel();
    if (isNativeAppPerformanceDetailDashboardLabel(label)) return;
    try {
      await this.selectDashboard(NAPD_DASH_EXACT);
      await this.page.waitForTimeout(5000);
    } catch {
      await this.selectDashboard(/^\s*Native App Performance Detail\s*$/i).catch(() => undefined);
      await this.page.waitForTimeout(4500);
    }
    label = await this.getDashboardLabel();
    if (isNativeAppPerformanceDetailDashboardLabel(label)) {
      await this.ensureProfileSiteSelected();
      return;
    }
    if (soft) return;
    ...
    await this.waitForPageReady();
    await this.selectDashboard(NAPD_DASH_EXACT).catch(() => undefined);
    await this.page.waitForTimeout(5000);
    await this.ensureProfileSiteSelected();
    label = await this.getDashboardLabel();
    expect(label, 'Dashboard switcher should show exact Native App Performance Detail').toMatch(NAPD_DASH_EXACT);
  }
  ...
  async selectDashboard(name: string | RegExp): Promise<void> {
    ...
    const re =
      typeof name === 'string'
        ...
        : name;
    const preferExactNapd =
      typeof name === 'string'
        ? isNativeAppPerformanceDetailDashboardLabel(name)
        : /Native App Performance Detail/i.test(String(name)) &&
          !/Detailed Metrics/i.test(String(name));
    ...
    if (await container.isVisible().catch(() => false)) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (preferExactNapd && !isNativeAppPerformanceDetailDashboardLabel(text)) continue;
        await options.nth(i).click({ force: true });
        clicked = true;
        break;
      }
      if (!clicked && typeof name === 'string') {
        // Exact string match (case-insensitive) via visible option text
        const exact = options
          .filter({ hasText: new RegExp(`^\\s*${name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\s*$`, 'i') })
          .first();
    ...
    const selectEl = this.locators.switchDashboard;
    const labels = await selectEl.locator('option').allTextContents();
    const normalized = labels.map((t) => t.replace(/\s+/g, ' ').trim());
    const match =
      (preferExactNapd
        ? normalized.find((t) => isNativeAppPerformanceDetailDashboardLabel(t))
        : undefined) ||
      normalized.find((t) => re.test(t)) ||
      (preferExactNapd
        ? undefined
        : normalized.find((t) => typeof name === 'string' && t.toLowerCase() === name.toLowerCase())) ||
      (preferExactNapd
        ? normalized.find((t) => isNativeAppPerformanceDetailDashboardLabel(t))
        : undefined);
    if (!match) throw new Error(`Dashboard option not found: ${name}`);
    await selectEl.selectOption({ label: match }).catch(async () => {
      const val = await selectEl.locator('option').filter({ hasText: re }).first().getAttribute('value');
      if (val) await selectEl.selectOption(val);
    });
    await this.page.waitForTimeout(6000);
    await this.ensureProfileSiteSelected();
  }
  ...
  async widgetsReadyScore(): Promise<number> {
    ...
    if (/PAGE VIEWS|Performance Timing|PERFORMANCE TIMING|Errors and Crashes|ERRORS AND CRASHES/i.test(body)) score += 1;
    ...
    if (
      ...
      score += 1;
    if (/CPU Usage|Memory Usage|Errors and Crashes|PERFORMANCE TIMING|Performance Timing \/ Onload/i.test(body)) score += 1;
    return score;
  }
  ...
  async captureContext(): Promise<NativeAppPerformanceDetailDashboardContext> {
    return {
  ...
  async restoreContext(ctx: NativeAppPerformanceDetailDashboardContext): Promise<void> {
    await this.closeOverlays();
    await this.ensureNativeAppPerformanceDetailSelected();
    await this.ensureProfileSiteSelected();
    ...
    // Restore Details metric toward Performance Timing when possible
    await this.clickMetricCard(/PERFORMANCE TIMING|Performance Timing/i).catch(() => undefined);
    await this.expectCoreWidgetsReady().catch(() => undefined);
  }
  ...
  async expectDetailsMetricCards(): Promise<string[]> {
    ...
    // GDC probe home: three native-oriented core cards (extras soft)
    const expected: Array<{ name: string; re: RegExp; core?: boolean }> = [
      { name: 'Page Views', re: /PAGE VIEWS|Page Views/i, core: true },
      { name: 'Performance Timing', re: /PERFORMANCE TIMING|Performance Timing/i, core: true },
      { name: 'Errors and Crashes', re: /ERRORS AND CRASHES|Errors and Crashes/i, core: true },
      { name: 'CPU Usage', re: /CPU Usage|CPU USAGE/i },
      { name: 'Memory Usage', re: /Memory Usage|MEMORY USAGE/i },
      { name: 'Onload', re: /ONLOAD|\bOnload\b/i },
    ];
    const found: string[] = [];
    const coreFound: string[] = [];
    for (const e of expected) {
      if (e.re.test(body)) {
        found.push(e.name);
        if (e.core) coreFound.push(e.name);
      }
    }
    expect(
      coreFound.length,
      `Core Details cards (Page Views / Performance Timing / Errors and Crashes); body: ${body.slice(0, 280)}`
  ...
  async softMetricCardLinkage(): Promise<{ cards: string[]; note: string }> {
    ...
    await tryCard('PERFORMANCE TIMING', /PERFORMANCE TIMING|Performance Timing/i);
    await tryCard('ERRORS AND CRASHES', /ERRORS AND CRASHES|Errors and Crashes/i);

    const afterDaily = await this.getDailyAveragesSignature();
    const afterBar = await this.getBarGraphSignature();
    // Restore Performance Timing
    await this.clickMetricCard(/PERFORMANCE TIMING|Performance Timing/i).catch(() => undefined);
  ...
  async softPerformanceLegendToggle(): Promise<{ toggled: boolean; note: string }> {
    ...
    if (count < 1) {
      ...
      // Prefer native-oriented legend series when available
      const byName = widget
        .getByText(/CPU Usage|Memory Usage|Errors and Crashes|Performance Timing|Group \/ Page Views|Page Views|Onload/i)
        .first();
  ...
  async softPlusDashboardEyeSwitchHome() {
    ...
    let r = await softPlusDashboardEyeSwitch(this.page, NAPD_DASH_EXACT);
    if (!r.restoredHome) {
      await this.ensureNativeAppPerformanceDetailSelected({ soft: true });
      r = {
        ...r,
        restoredHome: isNativeAppPerformanceDetailDashboardLabel(await this.getDashboardLabel()),
      };
    }
    await this.ensureProfileSiteSelected();
    await this.expectCoreWidgetsReady().catch(() => undefined);
  ...
  async expectNotConfusedSurfaces(): Promise<void> {
    ...
    // Must stay on Dashboards shell (not Monitoring native PD deep path as route home)
    ...
    expect(dash).not.toMatch(/^\s*RUM Performance Detail/i);
    expect(dash).not.toMatch(/^\s*Synthetic Performance Detail/i);
    expect(dash).not.toMatch(/Detailed Metrics/i);
    expect(dash).not.toMatch(/Traffic Source and Medium/i);
  }

  /** Soft: switch RUM PD (or Synthetic) then restore NAPD — exact name discrimination. */
  async softRumOrSyntheticSiblingDiscrimination(): Promise<{ note: string; restored: boolean }> {
    const names = await this.listPreconfiguredDashboardNames();
    const sibling =
      names.find((n) => /^\s*RUM Performance Detail\s*$/i.test(n)) ||
      names.find((n) => /^\s*Synthetic Performance Detail\s*$/i.test(n));
    if (!sibling) {
      return { note: 'RUM/Synthetic Performance Detail not in switcher', restored: true };
    }
    await this.selectDashboard(sibling);
    await this.page.waitForTimeout(3000);
    const after = await this.getDashboardLabel();
    await this.ensureNativeAppPerformanceDetailSelected();
    await this.ensureProfileSiteSelected();
    const restored = isNativeAppPerformanceDetailDashboardLabel(await this.getDashboardLabel());
    return {
      note: `Visited sibling="${after}"; restoredNAPD=${restored}`,
      restored,
    };
  }

  /** Soft: switch Native App Detailed Metrics - Android then restore NAPD home. */
  async softDetailedMetricsSiblingDiscrimination(): Promise<{ note: string; restored: boolean }> {
    const names = await this.listPreconfiguredDashboardNames();
    const sibling =
      names.find((n) => /Native App Detailed Metrics\s*-\s*Android/i.test(n)) ||
      names.find((n) => /Native App Detailed Metrics\s*-\s*iOS/i.test(n));
    if (!sibling) {
      return { note: 'Native App Detailed Metrics sibling not in switcher', restored: true };
    }
    await this.selectDashboard(sibling);
    await this.page.waitForTimeout(3500);
    const after = await this.getDashboardLabel();
    expect(isNativeAppPerformanceDetailDashboardLabel(after), `must not treat Detailed Metrics as NAPD: "${after}"`).toBeFalsy();
    await this.ensureNativeAppPerformanceDetailSelected();
    await this.ensureProfileSiteSelected();
    const restored = isNativeAppPerformanceDetailDashboardLabel(await this.getDashboardLabel());
    return {
      note: `Visited Detailed Metrics="${after}"; restoredNAPD=${restored}`,
      restored,
    };
  }
}
