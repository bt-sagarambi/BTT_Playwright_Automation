...
import { MarketingRegressionUs2DashboardLocators } from '../locators/MarketingRegressionUs2DashboardLocators';
...
export const MR_US2_EXACT = /^\s*Marketing Regression\s*-\s*US2\s*$/i;

export type MarketingRegressionUs2Context = {
...
/**
 * Custom Marketing Regression - US2 dashboard (site/dashboard).
 * Read-only: no Save Filter / widget Save / Delete widget / RO sticky device tab.
 */
export class MarketingRegressionUs2DashboardPage {
  readonly locators: MarketingRegressionUs2DashboardLocators;

  constructor(private readonly page: Page) {
    this.locators = new MarketingRegressionUs2DashboardLocators(page);
  }
  ...
  async openViaNavigation(): Promise<void> {
    ...
    await this.ensureMarketingRegressionUs2Selected();
  ...
  async recoverPage(): Promise<void> {
    ...
    await this.ensureMarketingRegressionUs2Selected({ soft: true });
    await this.ensureProfileSiteSelected();
    if (!MR_US2_EXACT.test(await this.getDashboardLabel())) {
      await this.openViaNavigation();
    } else {
  ...
  async ensureMarketingRegressionUs2Selected(opts?: { soft?: boolean }): Promise<void> {
    const soft = opts?.soft === true;
    let label = await this.getDashboardLabel();
    if (MR_US2_EXACT.test(label)) return;
    try {
      await this.selectDashboard(MR_US2_EXACT);
      await this.page.waitForTimeout(5000);
    } catch {
      await this.selectDashboard(/Marketing Regression\s*-\s*US2/i).catch(() => undefined);
    ...
    await this.waitForPageReady();
    await this.selectDashboard(MR_US2_EXACT).catch(() => undefined);
    await this.page.waitForTimeout(5000);
    label = await this.getDashboardLabel();
    expect(label, 'Dashboard switcher should show exact Marketing Regression - US2').toMatch(MR_US2_EXACT);
  }

  async selectDashboard(name: string | RegExp): Promise<void> {
    await this.closeOverlays();
    const re = typeof name === 'string' ? new RegExp(name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'i') : name;
    const preferHome = /Marketing Regression/i.test(String(name));
    ...
    if (await container.isVisible().catch(() => false)) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (preferHome && !MR_US2_EXACT.test(text)) continue;
      ...
      if (!clicked) {
        ...
        if (await opt.isVisible({ timeout: 3000 }).catch(() => false)) {
          ...
          if (!preferHome || MR_US2_EXACT.test(t)) {
            await opt.click({ force: true });
            clicked = true;
          }
        }
    ...
    const match =
      labels.find((t) => MR_US2_EXACT.test(t.replace(/\s+/g, ' ').trim()) && preferHome) ||
  ...
  async expectCoreWidgetTitles(): Promise<void> {
    ...
    expect(body, 'Campaign Information - US2').toMatch(/Campaign Information(\s*-\s*US2)?/i);
    expect(body, 'Bottom of Sales Funnel - US2').toMatch(/Bottom\s+of\s+(the\s+)?Sales\s+Funnel(\s*-\s*US2)?/i);
  ...
  async widgetsReadyScore(): Promise<number> {
    ...
    if (/Campaign Information/i.test(body)) score += 1;
    if (/Bottom\s+of\s+(the\s+)?Sales\s+Funnel/i.test(body)) score += 1;
    if (/Revenue Opportunity/i.test(body)) score += 1;
    const charts = await this.locators.highchartsContainers.count().catch(() => 0);
    if (charts >= 1) score += 1;
    if (/Page Views|Sessions|Bottom Funnel Conversion|View By/i.test(body)) score += 1;
    if (/ALL|DESKTOP|Revenue Opportunity by Page/i.test(body)) score += 1;
    return score;
  }
  ...
  async captureGraphLookbackSignature(): Promise<string> {
    return this.page.evaluate(() => {
      ...
      const campaign = find(/Campaign Information/i);
      const funnel = find(/Bottom\s+of\s+(the\s+)?Sales\s+Funnel/i);
      ...
      return [
        `hc=${charts}`,
        txt(campaign).slice(0, 160),
        txt(funnel).slice(0, 180),
        txt(ro).slice(0, 140),
      ].join('||');
    });
  }

  async getCampaignLabel(): Promise<string> {
    const name = this.locators.campaignName();
    if (await name.count().catch(() => 0)) {
      return ((await name.textContent().catch(() => '')) || '').replace(/\s+/g, ' ').trim();
    }
    const sample = await this.widgetSample(this.locators.campaignInformationWidget(), 220);
    const m = sample.match(/\(No Campaign Assigned\)|All Traffic|[A-Za-z0-9 _-]{3,60}/);
    return (m?.[0] || '').trim();
  }

  async getFunnelViewByMode(): Promise<string> {
    const sample = await this.widgetSample(this.locators.bottomOfSalesFunnelWidget(), 400);
    if (/View By:\s*Sessions|Sessions\s+active|active.*Sessions/i.test(sample) && !/View By:\s*Page Views/i.test(sample)) {
      return 'Sessions';
    }
    return this.page.evaluate(() => {
      const root = document.querySelector('[id^="sales-funnel-page-views-sessions-selector-"]');
      const active = root?.querySelector('.active, .selected, [aria-pressed="true"]');
      const t = ((active?.textContent || root?.textContent || '') as string).replace(/\s+/g, ' ').trim();
      if (/Sessions/i.test(t) && !/Page Views/i.test(t)) return 'Sessions';
      return 'Page Views';
    }).catch(() => 'Page Views');
  }

  async softFunnelViewByRoundTrip(): Promise<string> {
    await this.softScrollTo(this.locators.bottomOfSalesFunnelWidget());
    const start = await this.getFunnelViewByMode();
    const notes: string[] = [`start=${start}`];
    const selector = this.locators.funnelViewBySelector();
    const widget = this.locators.bottomOfSalesFunnelWidget();
    const clickMode = async (mode: RegExp) => {
      const hosts = [selector, widget];
      for (const host of hosts) {
        if (!(await host.count().catch(() => 0))) continue;
        const btn = host.locator('button, a, label, input, span').filter({ hasText: mode }).first();
        if (await btn.isVisible().catch(() => false)) {
      ...
      // Force-click via evaluate on known selector text nodes
      const clicked = await this.page.evaluate((wantSessions) => {
        const root =
          document.querySelector('[id^="sales-funnel-page-views-sessions-selector-"]') ||
          [...document.querySelectorAll('.grid-stack-item')].find((el) =>
            /Bottom\s+of\s+(the\s+)?Sales\s+Funnel/i.test(el.textContent || '')
          );
        if (!root) return false;
        const re = wantSessions ? /^\s*Sessions\s*$/i : /^\s*Page Views\s*$/i;
        for (const el of [...root.querySelectorAll('button, a, label, span, div, input')]) {
        ...
        return false;
      }, /Sessions/i.test(mode.source));
      if (clicked) await this.page.waitForTimeout(800);
      return clicked;
    };
    if (/Page Views/i.test(start)) {
      if (await clickMode(/Sessions/i)) notes.push('toggled Sessions');
      if (await clickMode(/Page Views/i)) notes.push('restored Page Views');
    } else {
      if (await clickMode(/Page Views/i)) notes.push('toggled Page Views');
      if (await clickMode(/Sessions/i)) notes.push('restored Sessions');
    }
    notes.push(`end=${await this.getFunnelViewByMode()}`);
    return notes.join('; ');
  }

  async softCampaignPickerSampleRestore(): Promise<string> {
    await this.softScrollTo(this.locators.campaignInformationWidget());
    const before = await this.getCampaignLabel();
    const btn = this.locators.campaignButton().or(this.locators.campaignName()).first();
    if (!(await btn.isVisible().catch(() => false))) {
      return `picker not visible; label="${before.slice(0, 60)}"`;
    }
    // Soft open/close only — campaign dropdown can be heavy; never Save and never leave sticky.
    await btn.click({ force: true, timeout: 4000 }).catch(() => undefined);
    await this.page.waitForTimeout(400);
    const menuVisible = await this.page
      .locator('.select2-results__option, .dropdown-menu:visible, .ui-menu:visible')
      .first()
      .isVisible()
      .catch(() => false);
    await this.page.keyboard.press('Escape').catch(() => undefined);
    await this.closeOverlays();
    const after = await this.getCampaignLabel();
    return `opened=${menuVisible} before="${before.slice(0, 40)}" after="${after.slice(0, 40)}" (sample-restore soft; no sticky Save)`;
  }

  async captureContext(): Promise<MarketingRegressionUs2Context> {
    return {
      ...
      campaignLabel: await this.getCampaignLabel().catch(() => ''),
      funnelViewBy: await this.getFunnelViewByMode().catch(() => 'Page Views'),
    };
  }

  async restoreContext(ctx: MarketingRegressionUs2Context): Promise<void> {
    await this.closeOverlays();
  ...
  > {
    ...
    for (const label of labels) {
      ...
      try {
        ...
        tip = await this.softHoverWidget(this.locators.bottomOfSalesFunnelWidget());
        if (!tip) {
          const pv = this.locators.funnelPageViewsChart();
          if (await pv.count().catch(() => 0)) tip = await this.softHoverWidget(pv);
        }
        if (!tip) {
          const sess = this.locators.funnelSessionsChart();
          if (await sess.count().catch(() => 0)) tip = await this.softHoverWidget(sess);
        }
    ...
    if (nonempty.length >= 2) {
      ...
      out.push({
        ...
        note:
          uniq.size >= 2
            ? `soft: differing funnel tooltip/interval signatures across lookbacks (unique≈${uniq.size})`
      ...
      out.push({
        ...
        tip: '',
        signature: '',
        note: 'soft: insufficient hoverable funnel points across lookbacks — controlled empty annotate',
      });
    }
  ...
  async expectNotConfusedSurfaces(): Promise<void> {
    ...
    await expect(this.page).not.toHaveURL(/bottom-sales-funnel|path-type=funnel/i);
    ...
    expect(dash).toMatch(MR_US2_EXACT);
    ...
    expect(dash).not.toMatch(/^(RUM|Tag Governance) Regression\s*-\s*US2$/i);
