...
import { SyntheticPerformanceDetailDashboardLocators } from '../locators/SyntheticPerformanceDetailDashboardLocators';
...
/** Exact preconfigured option — never RUM / Site Health / Native App / partial Performance Detail. */
export const SPD_DASH_EXACT = /^\s*Synthetic Performance Detail\s*$/i;
export const SPD_DASH_ALT = /^\s*Synthetic\s+Performance\s+Detail\s+Dashboard\s*$/i;

export function isSyntheticPerformanceDetailDashboardLabel(text: string): boolean {
  const t = (text || '').replace(/\s+/g, ' ').trim();
  if (/\bRUM\b|site\s*health|native\s*app|overview|aggregate/i.test(t)) return false;
  return SPD_DASH_EXACT.test(t) || SPD_DASH_ALT.test(t);
}

export type SyntheticPerformanceDetailDashboardContext = {
...
/**
 * Preconfigured Synthetic Performance Detail dashboard (site/dashboard).
 * Read-only regression: no Save Filter / permanent widget save / dashboard delete.
 * NOT Monitoring → Synthetic → Real Browser → Performance Detail.
 */
export class SyntheticPerformanceDetailDashboardPage {
  readonly locators: SyntheticPerformanceDetailDashboardLocators;

  constructor(private readonly page: Page) {
    this.locators = new SyntheticPerformanceDetailDashboardLocators(page);
  }
  ...
  async openViaNavigation(): Promise<void> {
    ...
    await this.ensureSyntheticPerformanceDetailSelected();
  ...
  async recoverPage(): Promise<void> {
    ...
    await this.ensureSyntheticPerformanceDetailSelected({ soft: true });
    await this.ensureProfileSiteSelected();
    if (!isSyntheticPerformanceDetailDashboardLabel(await this.getDashboardLabel())) {
      await this.openViaNavigation();
    } else {
  ...
  async ensureSyntheticPerformanceDetailSelected(opts?: { soft?: boolean }): Promise<void> {
    const soft = opts?.soft === true;
    let label = await this.getDashboardLabel();
    if (isSyntheticPerformanceDetailDashboardLabel(label)) return;
    try {
      await this.selectDashboard(SPD_DASH_EXACT);
      await this.page.waitForTimeout(5000);
    } catch {
      await this.selectDashboard(/^\s*Synthetic Performance Detail\s*$/i).catch(() => undefined);
      await this.page.waitForTimeout(4500);
    }
    label = await this.getDashboardLabel();
    if (isSyntheticPerformanceDetailDashboardLabel(label)) {
      await this.ensureProfileSiteSelected();
      return;
    }
    if (soft) return;
    ...
    await this.waitForPageReady();
    await this.selectDashboard(SPD_DASH_EXACT).catch(() => undefined);
    await this.page.waitForTimeout(5000);
    await this.ensureProfileSiteSelected();
    label = await this.getDashboardLabel();
    expect(label, 'Dashboard switcher should show exact Synthetic Performance Detail').toMatch(
      SPD_DASH_EXACT
    );
  }
  ...
  async selectDashboard(name: string | RegExp): Promise<void> {
    ...
    const re =
      typeof name === 'string'
        ...
        : name;
    const preferExactSpd = /Synthetic Performance Detail/i.test(String(name));
    ...
    if (await container.isVisible().catch(() => false)) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        if (preferExactSpd && !isSyntheticPerformanceDetailDashboardLabel(text)) continue;
        // Always reject RUM / site health / native when seeking Synthetic PD
        if (preferExactSpd && /\bRUM\b|site\s*health|native\s*app/i.test(text)) continue;
        await options.nth(i).click({ force: true });
        clicked = true;
        break;
      }
      if (!clicked && !preferExactSpd) {
        const opt = options.filter({ hasText: re }).first();
        if (await opt.isVisible({ timeout: 3000 }).catch(() => false)) {
          const t = ((await opt.textContent()) || '').replace(/\s+/g, ' ').trim();
          if (!/\bRUM\b|site\s*health|native\s*app/i.test(t) || !/Performance Detail/i.test(String(name))) {
            await opt.click({ force: true });
            clicked = true;
          }
        }
    ...
    const match =
      labels.find(
        (t) =>
          isSyntheticPerformanceDetailDashboardLabel(t.replace(/\s+/g, ' ').trim()) && preferExactSpd
      ) ||
      labels.find(
        (t) =>
          re.test(t.replace(/\s+/g, ' ').trim()) &&
          !/\bRUM\b|site\s*health|native\s*app/i.test(t) &&
          !preferExactSpd
      ) ||
      labels.find((t) => isSyntheticPerformanceDetailDashboardLabel(t.replace(/\s+/g, ' ').trim()));
  ...
  async captureContext(): Promise<SyntheticPerformanceDetailDashboardContext> {
    return {
  ...
  async restoreContext(ctx: SyntheticPerformanceDetailDashboardContext): Promise<void> {
    await this.closeOverlays();
    await this.ensureSyntheticPerformanceDetailSelected();
    await this.ensureProfileSiteSelected();
    ...
    await this.clickMetricCard(/PAGE ONLOAD|ONLOAD|\bOnload\b/i).catch(() => undefined);
    await this.expectCoreWidgetsReady().catch(() => undefined);
  }
  ...
  async expectDetailsMetricCards(): Promise<string[]> {
    ...
    const expected: Array<{ name: string; re: RegExp }> = [
      ...
      { name: 'Page Onload', re: /PAGE ONLOAD|ONLOAD|\bOnload\b/i },
      ...
      },
      {
        ...
        re: /FIRST CONTENTFUL PAINT|First Contentful Paint|\bFCP\b/i,
      },
      { name: 'Speed Index', re: /SPEED INDEX|Speed Index/i },
      { name: 'First Byte', re: /FIRST BYTE|First Byte|1stByte|TTFB/i },
      { name: 'JS Errors', re: /JS ERRORS|JS Errors \(Sampled\)/i },
      {
        name: 'Page Size',
        re: /PAGE SIZE \(UNCOMPRESSED\)|PAGE SIZE \(COMPRESSED\)|Page Size/i,
      },
      {
        name: 'Total Elements/Domains',
        re: /TOTAL ELEMENTS|TOTAL DOMAINS|Total Elements|Total Domains/i,
      },
      { name: 'Cumulative Layout Shift', re: /CUMULATIVE LAYOUT SHIFT|\bCLS\b/i },
  ...
  async softMetricCardLinkage(): Promise<{ cards: string[]; note: string }> {
    ...
    await tryCard('PAGE ONLOAD', /PAGE ONLOAD|ONLOAD|\bOnload\b/i);
    await tryCard(
      'LCP_OR_SPEED_OR_SIZE',
      /LARGEST CONTENTFUL PAINT|SPEED INDEX|PAGE SIZE \(UNCOMPRESSED\)/i
    );

    const afterDaily = await this.getDailyAveragesSignature();
    const afterBar = await this.getBarGraphSignature();
    // Restore Onload / Page Onload
    await this.clickMetricCard(/PAGE ONLOAD|ONLOAD|\bOnload\b/i).catch(() => undefined);
  ...
  async softPlusDashboardEyeSwitchHome() {
    ...
    let r = await softPlusDashboardEyeSwitch(this.page, SPD_DASH_EXACT);
    if (!r.restoredHome) {
      await this.ensureSyntheticPerformanceDetailSelected({ soft: true });
      r = {
        ...r,
        restoredHome: isSyntheticPerformanceDetailDashboardLabel(await this.getDashboardLabel()),
      };
    }
    await this.ensureProfileSiteSelected();
    await this.expectCoreWidgetsReady().catch(() => undefined);
  ...
  async expectNotConfusedSurfaces(): Promise<void> {
    ...
    // Must not be Monitoring Performance Detail deep path (RUM or Synthetic monitoring modules)
  ...
  /** Soft probe: switch RUM Performance Detail then restore — layout sibling discrimination. */
  async softRumSiblingDiscrimination(): Promise<{ note: string; restored: boolean }> {
    const names = await this.listPreconfiguredDashboardNames();
    const rum = names.find((n) => /^\s*RUM Performance Detail\s*$/i.test(n));
    if (!rum) {
      return { note: 'RUM Performance Detail not in switcher', restored: true };
    }
    await this.selectDashboard(/^\s*RUM Performance Detail\s*$/i);
    await this.page.waitForTimeout(3000);
    const after = await this.getDashboardLabel();
    await this.ensureSyntheticPerformanceDetailSelected();
    await this.ensureProfileSiteSelected();
    const restored = isSyntheticPerformanceDetailDashboardLabel(await this.getDashboardLabel());
    return {
      note: `Visited sibling="${after}"; restoredSyntheticPD=${restored}`,
      restored,
    };
  }

  /** Soft probe: switch Synthetic Site Health then restore — name sibling discrimination. */
  async softSiteHealthSiblingDiscrimination(): Promise<{ note: string; restored: boolean }> {
    const names = await this.listPreconfiguredDashboardNames();
    const ssh = names.find((n) => /^\s*Synthetic Site Health\s*$/i.test(n));
    if (!ssh) {
      return { note: 'Synthetic Site Health not in switcher', restored: true };
    }
    await this.selectDashboard(/^\s*Synthetic Site Health\s*$/i);
    await this.page.waitForTimeout(3000);
    const after = await this.getDashboardLabel();
    await this.ensureSyntheticPerformanceDetailSelected();
    await this.ensureProfileSiteSelected();
