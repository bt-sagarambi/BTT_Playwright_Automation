...
/**
 * Regression: Native App Detailed Metrics - iOS (preconfigured dashboard)
 * Site: GDC Test Site 2
 * Path: tests/regression_tests/US2/dashboards/preconfigured/native-app-detailed-metrics-ios
 *
 * Suite home: Native App Detailed Metrics - iOS
 * Soft dual: Native App Detailed Metrics - Android
 *
 ...
 *
 * npm: test:regression:us2:native-app-detailed-metrics-ios-dashboard
 */
...
test.describe('US2 Regression — Native App Detailed Metrics - iOS Dashboard', () => {
  ...
  const annotate = (description: string) => {
    ...
    console.log(`[NADM-IOS-DASH] ${description}`);
  };

  const recover = async () => {
    await nadm.recoverPage('ios');
  ...
  test.beforeAll(async ({ browser }, testInfo) => {
    ...
    await nadm.openViaNavigation('ios');
    ...
    initialCtx = await nadm.captureContext('ios');
    const profile = getActiveProfile();
    console.log(
      `[NADM-IOS-DASH] profile=${profile.id} site=${await nadm.getSiteLabel().catch(() => profile.siteName)} loadMs=${initialLoadMs} dashboard="${initialCtx.dashboardLabel}" lookback="${initialCtx.lookbackLabel}"`
    );
  });

  test.afterAll(async () => {
    if (notes.length) console.log(`[NADM-IOS-DASH] annotations:\n- ${notes.join('\n- ')}`);
    await page?.context()?.close().catch(() => undefined);
  });

  test('REG-NADM-IOS-DASH-001 — page loads via Dashboards menu/route with correct title', async () => {
    ...
    await nadm.expectNotConfusedSurfaces('ios');
  });

  test('REG-NADM-IOS-DASH-002 — exact iOS Detailed Metrics; multi-widget inventory', async () => {
    await nadm.ensureOsSelected('ios');
    const dash = await nadm.getDashboardLabel();
    expect(isNativeAppDetailedMetricsLabel(dash, 'ios'), `dash="${dash}"`).toBeTruthy();
    expect(dash).toMatch(NADM_IOS_EXACT);
  ...
  test('REG-NADM-IOS-DASH-003 — GDC Test Site 2; lookback/auto-refresh captured', async () => {
    await nadm.ensureProfileSiteSelected();
  ...
  test('REG-NADM-IOS-DASH-004 — dashboard chrome: switcher, lookback, refresh, auto-refresh', async () => {
  ...
  test('REG-NADM-IOS-DASH-005 — Preconfigured list has Android + iOS Detailed Metrics', async () => {
  ...
  test('REG-NADM-IOS-DASH-006 — Time Lookback menu multi presets', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-007 — soft apply Last 24h then Last 7d; restore lookback', async () => {
    try {
    ...
    await expect(nadm.getDashboardLabel()).resolves.toMatch(NADM_IOS_EXACT);
  });

  test('REG-NADM-IOS-DASH-008 — Dashboard Refresh reloads without blank crash', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-009 — Auto Refresh menu soft (Off, 2m…60m)', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-010 — Native App Performance shell + series soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-011 — Performance legend toggle soft', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-012 — 1st vs 3rd Party Activity - Native shell soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-013 — party Domains ↔ Network Requests toggle soft', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-014 — Network Request Failures Over Time shell + legend soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-015 — iOS Friction Map title; empty soft', async () => {
    const friction = await nadm.softAssertFrictionOs('ios');
    expect(friction.ok || /iOS Friction/i.test(friction.sample)).toBeTruthy();
    annotate(`Friction: ${friction.sample}`);
    for (const n of await nadm.softEmptyAnnotation()) annotate(n);
  });

  test('REG-NADM-IOS-DASH-016 — Crash Summary shell; empty soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-017 — Native App HTTP Responses tiles soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-018 — HTTP Responses By Error Count shell soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-019 — ANR & OOM warnings shell; empty soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-020 — CPU and Memory Usage shell + table headers soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-021 — CPU graph/table toggle soft', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-022 — Filters drawer + My/Shared tabs soft', async () => {
    try {
      await withSoftDeadline(
        async () => {
  ...
  test('REG-NADM-IOS-DASH-023 — Filters Apply soft; restore iOS home', async () => {
    try {
      await withSoftDeadline(
        async () => {
          await nadm.openFiltersDrawer();
  ...
  test('REG-NADM-IOS-DASH-024 — Dashboard Manager soft open/close', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-025 — Add Widget wizard soft cancel', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-026 — Android dual coverage then restore iOS home', async () => {
    try {
      const r = await withSoftDeadline(() => nadm.softAndroidRoundTrip(), 120000, recover);
      annotate(r.note);
      expect(r.restored || isNativeAppDetailedMetricsLabel(await nadm.getDashboardLabel(), 'ios')).toBeTruthy();
      await expect(nadm.getDashboardLabel()).resolves.toMatch(NADM_IOS_EXACT);
      await nadm.expectSelectedSite();
      if (!r.frictionAndroid) annotate('Android friction title soft annotate: may be empty chrome');
    } catch (err) {
      annotate(`Android dual soft: ${err instanceof Error ? err.message : String(err)}`);
      await recover();
    }
  });

  test('REG-NADM-IOS-DASH-027 — soft sibling non-native then restore iOS NADM', async () => {
    try {
      await withSoftDeadline(
        async () => {
          const names = await nadm.listPreconfiguredDashboardNames();
      ...
      await nadm.openViaNavigation('ios').catch(() => recover());
    }
  });

  test('REG-NADM-IOS-DASH-028 — Native App Performance Detail discrimination then restore', async () => {
    try {
      const r = await withSoftDeadline(
        () => nadm.softPerformanceDetailSiblingDiscrimination('ios'),
        90000,
        recover
      );
      annotate(r.note);
      expect(
        r.restored || isNativeAppDetailedMetricsLabel(await nadm.getDashboardLabel(), 'ios')
      ).toBeTruthy();
      await expect(nadm.getDashboardLabel()).resolves.toMatch(NADM_IOS_EXACT);
    } catch (err) {
  ...
  test('REG-NADM-IOS-DASH-029 — Custom Date open/cancel only', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-030 — narrow viewport keeps widgets reachable', async () => {
  ...
  test('REG-NADM-IOS-DASH-031 — Escape recovery; Help chrome soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-032 — last-updated + Carousel soft', async () => {
  ...
  test('REG-NADM-IOS-DASH-033 — refresh keeps multi-widget inventory; still iOS NADM', async () => {
    await nadm.clickRefreshDashboard().catch(() => undefined);
    await nadm.expectCoreWidgetTitles();
    await expect(nadm.getDashboardLabel()).resolves.toMatch(NADM_IOS_EXACT);
  });

  test('REG-NADM-IOS-DASH-034 — history reload soft recovery', async () => {
    try {
      await withSoftDeadline(
        async () => {
  ...
  test('REG-NADM-IOS-DASH-035 — not Monitoring / NAPD-as-home / RUM-SPD / Marketing', async () => {
    await nadm.expectNotConfusedSurfaces('ios');
  ...
  test('REG-NADM-IOS-DASH-036 — top chrome icons soft round-trip', async () => {
    try {
      ...
      await nadm.ensureOsSelected('ios', { soft: true });
      await expect(page).toHaveURL(/site\/dashboard/i);
    } catch (err) {
  ...
  test('REG-NADM-IOS-DASH-037 — prescribed lookbacks 1h|6h / 24h / 7d / 30d', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-038 — site switch + lock soft; restore GDC', async () => {
    try {
      const r = await withSoftDeadline(() => nadm.softSiteChangeAndLock('ios'), 90000, recover);
      annotate(
  ...
  test('REG-NADM-IOS-DASH-039 — Auto Refresh apply sample; leave non-aggressive', async () => {
    try {
  ...
  test('REG-NADM-IOS-DASH-040 — +Dashboard/Manager eye then restore iOS NADM', async () => {
    try {
      const r = await withSoftDeadline(() => nadm.softPlusDashboardEyeSwitchHome('ios'), 90000, recover);
  ...
  test('REG-NADM-IOS-DASH-041 — restore initial context; suite home healthy', async () => {
    await nadm.restoreContext(initialCtx);
    await nadm.ensureOsSelected('ios');
    await nadm.expectSelectedSite();
    await nadm.expectCoreWidgetTitles();
    await nadm.expectNotConfusedSurfaces('ios');
    // Android option still listable (not selected unless dual mid-suite)
    const names = await nadm.listPreconfiguredDashboardNames();
    expect(names.some((n) => NADM_ANDROID_EXACT.test(n))).toBeTruthy();
