...
/**
 * Regression: Core Web Vitals (VitalScope)
 * Site: GDC Test Site 2 (tests/regression_tests/US2/business-insights/improve-traffic/core-web-vitals)
 *
 ...
 * Navigation: Business Insights > Improve Traffic > Core Web Vitals (VitalScope).
 * Copied from Monitoring Performance Overview; navigation steps only differ.
 */
...
test.describe('US2 Regression — Core Web Vitals (VitalScope)', () => {
  ...
  test.beforeAll(async ({ browser }) => {
    ...
    await rum.openViaCoreWebVitalsNavigation();
    const profile = getActiveProfile();
    console.log(`[Core Web Vitals] profile=${profile.id} site=${profile.siteName} dc=${profile.datacenter}`);
  });
  ...
  test('REG-CWV-001 — page loads via BI Core Web Vitals menu/route with correct title', async () => {
    await expect(rum.locators.pageTitle).toHaveText(
      /Business Insights\s*\/\s*Improve Traffic\s*\/\s*Core Web Vitals|VitalScope|Core Web Vitals|Performance Overview/i
    );
    await expect(page).toHaveURL(/real-user-monitoring\/performance-overview/);
  });

  test('REG-CWV-002 — default Performance by Page table and sections render', async () => {
    await rum.expectDefaultContext();
  });

  test('REG-CWV-003 — Performance by Page table headers include Page Name and CWV metrics', async () => {
    const headers = await rum.expectTableHeadersSample();
    console.log(`[CWV] headers sample: ${headers.slice(0, 8).join(' | ')}`);
  });

  test('REG-CWV-004 — top filter badges visible after View Filters', async () => {
  ...
  test('REG-CWV-018 — expand Top 50 URLs for a page', async () => {
  ...
  test('REG-CWV-019 — Copy URL icon copies expanded URL to clipboard', async () => {
    const url = await rum.copyFirstExpandedUrl();
    console.log(`[CWV] copied URL: ${url.slice(0, 120)}`);
  });

  test('REG-CWV-020 — Instant Measurement icon opens Synthetic Instant in new tab', async () => {
  ...
  test('REG-CWV-021 — expanded URL click opens Performance Detail in new tab', async () => {
  ...
  test('REG-CWV-022 — VitalScope metric icon opens drilldown modal with table', async () => {
    try {
  ...
  test('REG-CWV-023 — Customize Table opens with include/exclude/reset/default/save', async () => {
    await rum.openCustomizeTable();
    await expect(rum.locators.saveCustomizedTable).toHaveText(/Save\s*&\s*Close/i);
  });

  test('REG-CWV-024 — Customize Table Save & Close persists and returns to table', async () => {
  ...
  test('REG-CWV-025 — hamburger export exposes CSV download', async () => {
    try {
  ...
  test('REG-CWV-026 — Performance By Country section is present', async () => {
    try {
  ...
  test('REG-CWV-027 — Performance By Geography world map renders', async () => {
    try {
  ...
  test('REG-CWV-028 — Geography Zoom in / Zoom out controls respond', async () => {
    try {
  ...
  test('REG-CWV-029 — View Filters toggle shows or hides filter banner', async () => {
  ...
  test('REG-CWV-030 — right-nav Filters open and Apply Filters works (sample)', async () => {
    await rum.applySampleFilterCombo();
    await rum.expectDefaultContext();
  });

  test('REG-CWV-031 — Time Period Last 6 hours refreshes overview table', async () => {
  ...
  test('REG-CWV-032 — Time Period Last 24 hours refreshes overview table', async () => {
    test.setTimeout(150000);
    try {
      await withSoftDeadline(async () => {
  ...
  test('REG-CWV-033 — Time Period Last 7 days refreshes overview table', async () => {
    test.setTimeout(150000);
    try {
      await withSoftDeadline(async () => {
  ...
  test('REG-CWV-034 — Time Period Last 30 days refreshes overview table', async () => {
    test.setTimeout(150000);
    try {
      await withSoftDeadline(async () => {
  ...
  test('REG-CWV-035 — Favorites section optional soft-check via left nav', async () => {
