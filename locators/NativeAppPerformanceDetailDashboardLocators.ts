...
/**
 * Locators for preconfigured Native App Performance Detail dashboard.
 * Route: site/dashboard | menu: Dashboards | option: Native App Performance Detail
 *
 * Prefer stable control IDs and widget titles/headers.
 * Do not hard-code dynamic Highcharts instance IDs.
 *
 * NOT Monitoring → Native App → Performance Detail.
 * NOT Native App Detailed Metrics - Android/iOS.
 */
export class NativeAppPerformanceDetailDashboardLocators {
  ...
  readonly pageVolumeCard: Locator;
  readonly pageOnloadCard: Locator;
  readonly jsErrorCountCard: Locator;
  ...
  constructor(private readonly page: Page) {
    ...
    this.pageVolumeCard = page.locator('#page-volume-card');
    this.pageOnloadCard = page.locator('#page-onload-card');
    this.jsErrorCountCard = page.locator('#js-error-count-card');
  ...
  detailsWidget(): Locator {
    const byId = this.page.locator('.grid-stack-item').filter({
      has: this.page.locator(
        '#page-onload-card, #page-volume-card, #js-error-count-card, [id*="page-onload"], [class*="metric-card"]'
      ),
    });
  ...
  performanceGraphWidget(): Locator {
    const byId = this.page.locator('.grid-stack-item').filter({
      has: this.page.locator('#perfGraph-page-timings-graph, #page-timings-graph, [id^="perfGraph-"]'),
    });
    const byText = this.page
      .locator('.grid-stack-item')
  ...
  performanceGraphGear(): Locator {
    return this.performanceGraphWidget()
      .locator(
        'button, a, .btn, i.fa-cog, i.fa-gear, .glyphicon-cog, [class*="gear"], [title*="setting" i]'
      )
      .first();
  }
}
